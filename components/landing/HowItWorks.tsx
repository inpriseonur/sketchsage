...
const IconMap: Record<string, React.ReactNode> = {
  upload: (
    <svg className="w-12 h-12 text-[#D8753B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {steps.map((step, index) => (
            <div key={index} className="flex flex-col items-center gap-4">
              <div className="flex items-center justify-center size-24 bg-[#A45D40]/20 rounded-full border-2 border-[#D8753B]/50 p-4 backdrop-blur-sm">
                {IconMap[step.icon] || IconMap.lightbulb}
              </div>
              <div className="flex flex-col gap-1">
                <h3 className="text-lg font-bold">{step.order}. {step.title}</h3>
